...
- `scripts/project-verify.py verify --root <project> --knowledge-root <knowledge>` is the executable gate. `check` validates specification links, optional ADR links, packet provenance (including clean Git-worktree ancestry and changed-path ownership), dependency safety, declared evidence, and published-review traceability structurally; it does not execute arbitrary evidence strings or call hosts. `knowledge-index --check` and `knowledge-index --project --check` are non-mutating freshness gates for tracked Central and sparse Project indexes; `orchestrator-config-check` validates local executor/tracker/review-provider routing configuration.
