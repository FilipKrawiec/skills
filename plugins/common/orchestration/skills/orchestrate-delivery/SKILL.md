...
6. **REVIEW** — Open Pull Request (`gh pr create --issue <id>`) linking `Closes #<id>` and update project board to **Review**. Conduct multi-persona review (`quality-engineer`, `solution-architect`, `security-auditor`) auditing TDD assertion strength, DDD domain purity, Hexagonal layer isolation, and OWASP security risks. Update `walkthrough.md` and post PR review comments. Read [agent-personas.md](references/agent-personas.md). If a reviewer returns `CORRECT_EXECUTE` or `CORRECT_PLAN`, re-dispatch autonomously to the executor with correction payload until approved or retry limit (N <= 3) is reached.
7. **SHIP / RETURN** — Once changes pass all verifications and persona reviews, commit the verified changes, push the branch to remote origin (`git push`), and create exactly one durable **Delivery Record** (GitHub Issue / PR reference) for the cohesive delivery slice before publishing its **Review Request** on the short-lived task branch. Present the work to the Product Owner with interactive merge authorization instructions (`gh pr merge`). Upon merge, the issue auto-closes (`Closes #<id>`) and project item moves to **Done**. Executors do not merge, approve, or force-push protected default branches; the user retains merge authority.
