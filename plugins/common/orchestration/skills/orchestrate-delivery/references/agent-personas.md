...
This reference defines the canonical system prompts, tool capabilities, and return contracts for agent personas in the `orchestrate-delivery` workflow. These specifications are provider-neutral and reusable across Antigravity (`agy`), Claude Code, Codex, and custom AI CLI runners.
