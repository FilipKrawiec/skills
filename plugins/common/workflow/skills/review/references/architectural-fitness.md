...
| **Mandatory Application Traversal** | Inbound adapters must always route through Application use cases / query handlers (consistency over simplicity); never bypass the application layer to call domain repositories or ports directly. | API controller directly invokes a Domain repository, query port, or DAO without an Application use case. | Wrap domain/query invocation in an Application Use Case / Handler. |
