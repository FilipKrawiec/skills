...
2. For new or changed business behavior, run an EventStorming session before choosing bounded contexts, aggregates, or components. Capture the in-scope facts that can happen, order them into workflows and variants, and trace their commands, actors, policies, and external interactions. Record the result in `EVENT-STORMING.md` or the team's equivalent durable artifact.
3. Challenge ambiguous business terms with domain experts and cross-check them against the code. Record each resolved term in `CONTEXT.md`. Never put code file paths, database tables, or framework classes in `CONTEXT.md`.
