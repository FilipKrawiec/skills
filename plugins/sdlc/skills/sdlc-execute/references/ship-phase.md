...
2. Produce a `ShipmentCandidate` containing the schema fields in its YAML frontmatter, and details in markdown sections. Save it to `.sdlc/tasks/<task-id>/shipment.md` (and copy/symlink it as a host-scoped artifact if requested by the host). The file should contain:
...
3. Propose a successful `PhaseOutcome` of kind `Succeeded` containing:
   * `work_product`: The proposed `ShipmentCandidate` (represented by `shipment.md`).
