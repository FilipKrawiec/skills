...
1. **Locate the Skill's Absolute Path:**
   - Look at the `Available skills` list in your system prompt to find the absolute path of the active skill's `SKILL.md` file (e.g., `~/.gemini/config/plugins/filipkrawiec/plugins/core/skills/ddd/SKILL.md`).
...
2. **Construct the Reference's Absolute Path:**
   ...
   - *Example:* If the skill is `ddd` and the reference is `references/strategic-design.md`, the resolved absolute path is:
     `~/.gemini/config/plugins/filipkrawiec/plugins/core/skills/ddd/references/strategic-design.md`
